...
public interface MessageRepository extends JpaRepository<Message, Integer> {
	...
	@Query(value="SELECT * FROM tseam_six_3.message where fk_receiver_id=?1 and fk_sender_id=?2 or fk_receiver_id=?3 and fk_sender_id=?4 order by idmessage", nativeQuery = true)
