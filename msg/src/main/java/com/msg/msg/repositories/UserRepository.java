...
//	List<User> findTrainerByAreaAndType(String specialization_title, String city);
	...
	@Query(value = "select user.iduser, user.username,user.password,user.fk_role_id, user.first_name,user.last_name,user.email,user.price,user.description\r\n"
			+ " user.photolink from user, trainer_area,  trainer_specialization, area ,training_type\r\n"
	...
	@Query(value = "SELECT user.iduser,user.username,user.password,user.fk_role_id,user.first_name,user.last_name,user.email,user.price,user.description,user.photolink FROM user,training_type,trainer_specialization,trainer_area,area"
			+ " where user.iduser=trainer_specialization.fk_trainer_id and trainer_specialization.fk_training_type = training_type.idtraining_type and "
			+ "user.iduser=trainer_area.fk_trainer_id and trainer_area.fk_area_id=area.idarea and specialization_title = ?1 and"
	...
	@Query(value = "Select user.iduser,user.username,user.password,user.fk_role_id,user.first_name,user.last_name,user.email,user.price,user.description,user.photolink from user,trainer_area,area where iduser=fk_trainer_id and fk_area_id=idarea and idarea=?1", nativeQuery = true)
	List<User> findTrainerByArea(int idarea);

	@Query(value = "Select user.iduser,user.username,user.password,user.fk_role_id,user.first_name,user.last_name,user.email,user.price,user.description,user.photolink from user,trainer_area,area where iduser=fk_trainer_id and fk_area_id=idarea and idarea=?1 and price <=?2", nativeQuery = true)
	List<User> findTrainerByAreaAndPrice(int idarea, double price);

	@Query(value = "Select user.iduser,user.username,user.password,user.fk_role_id,user.first_name,user.last_name,user.email,user.price,user.description,user.photolink from user,trainer_specialization,training_type where iduser=fk_trainer_id and fk_training_type=idtraining_type and idtraining_type =?1", nativeQuery = true)
	List<User> findTrainerByType(int idtraining_type);

	@Query(value = "Select user.iduser,user.username,user.password,user.fk_role_id,user.first_name,user.last_name,user.email,user.price,user.description,user.photolink from user,trainer_specialization,training_type where iduser=fk_trainer_id and fk_training_type=idtraining_type and idtraining_type =?1 and price <=?2", nativeQuery = true)
