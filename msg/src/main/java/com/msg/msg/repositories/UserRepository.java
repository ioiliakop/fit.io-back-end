...
//	List<User> findTrainerByAreaAndType(String specialization_title, String city);
	...
	@Query(value = "Select user.iduser,user.username,user.password,user.fk_role_id,user.first_name,user.last_name,user.email,user.price,user.description from user,trainer_area,area where iduser=fk_trainer_id and fk_area_id=idarea and idarea=?1", nativeQuery = true)
	List<User> findTrainerByArea(int idarea);

	@Query(value = "Select user.iduser,user.username,user.password,user.fk_role_id,user.first_name,user.last_name,user.email,user.price,user.description from user,trainer_area,area where iduser=fk_trainer_id and fk_area_id=idarea and idarea=?1 and price <=?2", nativeQuery = true)
	List<User> findTrainerByAreaAndPrice(int idarea, double price);

	@Query(value = "Select user.iduser,user.username,user.password,user.fk_role_id,user.first_name,user.last_name,user.email,user.price,user.description from user,trainer_specialization,training_type where iduser=fk_trainer_id and fk_training_type=idtraining_type and idtraining_type =?1", nativeQuery = true)
	List<User> findTrainerByType(int idtraining_type);

	@Query(value = "Select user.iduser,user.username,user.password,user.fk_role_id,user.first_name,user.last_name,user.email,user.price,user.description from user,trainer_specialization,training_type where iduser=fk_trainer_id and fk_training_type=idtraining_type and idtraining_type =?1 and price <=?2", nativeQuery = true)
